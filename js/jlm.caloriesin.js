$(document).ready(function() {
	...
	function getdata(callback){
		...
		$.getJSON('json/caloriesin.json', function(b) {
			$( b ).each(function( index ) {
				item[index] = [];
				item[index][0] = b[index].itemdate;
				item[index][1] = b[index].itemvalue;
				item[index][2] = b[index].caloriesintype;
				item[index][3] = b[index].caloriesindesc;
				item[index][4] = b[index].caloriesinamount;
			});
			callback('hey');
		});
	}
	...
	function runitemloop() {
		if(debug==0){
			$('body').css('background-image', "url(images/food/" + encodeURIComponent(item[i][1].toLowerCase()) + ")");
		}else{
			$("#itemdiv").empty();

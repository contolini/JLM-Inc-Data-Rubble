$(document).ready(function() {
	...
	function getdata(callback){
		...
		$.getJSON('json/webhistory.json', function(b) {
			$( b ).each(function( index ) {
				item[index] = [];
				item[index][0] = b[index].itemdate;
				item[index][1] = b[index].webtitle;
				item[index][2] = b[index].webdomain;
				item[index][3] = b[index].webtype;
				item[index][4] = b[index].weburl;
			});
			callback('hey');
		});
	}
